...
class dt extends DateTime
{
    ...
    {
        if ($time && self::createFromDb($time)->setTimeZone(new DateTimeZone('UTC'))->getTimestamp() > 0) {
            return true;
        }
    }
...
{
    $deptsArr = (is_array($deptsNames)) ? $deptsNames : [$deptsNames];
    extract(t());
